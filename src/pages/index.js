...
export async function getStaticProps() {
  try {
    const [categoriesRes, storesRes, blogsRes] = await Promise.all([
      fetch(`https://admin.coupon.health/categories?ordering=title`, {
        headers: {
          'x-api-key': process.env.SECRET_KEY, // must be defined in .env.local
        },
      }),
      fetch(`https://admin.coupon.health/stores?ordering=title`, {
        headers: {
          'x-api-key': process.env.SECRET_KEY, // must be defined in .env.local
        },
      }),
      fetch(`https://admin.coupon.health/posts?ordering=-updated_at`, {
        headers: {
          'x-api-key': process.env.SECRET_KEY, // must be defined in .env.local
        },
      }),
